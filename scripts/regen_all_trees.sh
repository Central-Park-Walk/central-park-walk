...
for sp in $SPECIES; do
    for tier in $TIERS; do
        ...
        timeout 600 blender4 --background --python scripts/generate_trees_mtree.py -- --species "$sp" --tier "$tier" 2>&1 | tail -8
        if [ $? -ne 0 ]; then
...
timeout 120 blender4 --background --python scripts/generate_trees_mtree.py -- --species dead 2>&1 | tail -5
