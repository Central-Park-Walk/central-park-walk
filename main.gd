...
#
# SINGLE dense mesh + CONTINUOUS density falloff (2026-07-01, after the 3-ring experiment
# regressed): discrete LOD rings cross-faded ahead of the camera as a visible "shimmer /
# odd overlap" (Chris), and thinning the mid/far rings left the sward reading BARE at a
# grazing angle (far blades went sub-pixel → sparse crawling specks under FSR). Back to one
# mesh with a smooth per-blade rank falloff (dense near → 0 by TURF_RADIUS) so there are no
# ring boundaries and no far specks to crawl. Perf comes from a TIGHTER radius (blades fully
# gone by ~14 m, before they go sub-pixel) handing off to the shell/ground texture, not from
...
# rendered LOD tier: green lod0 (near) → impostor billboard, with
...
var _dist_tree_bands: PackedVector2Array = PackedVector2Array()  # parallel: (mesh_end, mesh_end) per tree (lod0→impostor handoff), for tier-true label colour
...
func _get_prof_data() -> Dictionary:
	...
	if _park_loader and _park_loader._tree_builder:
		...
		# LOD chain is lod0 → impostor (mid tier removed 2026-07-03).
		data["tree_impostor"] = tb.impostor_instances
		data["tree_impostor_chunks"] = tb.impostor_chunks
...
func _dist_cache_tree_positions() -> void:
	...
	# actual rendered tier (lod0 green / impostor blue-red), derived from the SAME
	# scaled handoff the lod_fade shaders use — not a fixed distance band.
...
func _setup_environment() -> void:
	...
	# SDFGI ON by default. Brief detour 2026-07-02: flipped OFF over black
	# under-canopy foliage (sdfgi_use_occlusion drives SDFGI's sky-ambient to ~0
	# under the dense canopy, and the leaf shader's 0.12 crown-interior AO multiplies
	# against ~0 → black). But that black only shows in a pathological look-STRAIGHT-
	# UP-into-canopy view; on a full walk Chris confirmed the normal eye-level SDFGI
	# look is the moody dappled forest he wants (cpw_009 OFF flat vs cpw_010/011 ON),
	# for only ~2fps + ~512MB VRAM. So SDFGI stays ON. Occlusion made NO visible
	# difference on the walk (F8 A/B, cpw_010≡011) so it's left at the standard true;
	# --sdfgi-no-occlusion / F8 remain if the up-view black ever needs the middle ground.
	# Opt out entirely with --no-sdfgi (or F2 live).
	_env.sdfgi_enabled         = not ("--no-sdfgi" in OS.get_cmdline_user_args())
...
func _setup_shell_grass() -> void:
	...
	# — the ground reads as continuous grass with no bare ring at the handoff. 0 = no clear
...
func _setup_turf_tiles() -> void:
	...
	## cross-fade shimmer). The grid extends past the fade-out so entering tiles are already
	## collapsed (no leading-edge "pulse"). Blades hand off to the shell/ground beyond.
...
func _apply_landuse_map(zones: Array, water: Array = []) -> void:
	## Load pre-baked landuse map (2048×2048; downsampled from 8192 for VRAM —
