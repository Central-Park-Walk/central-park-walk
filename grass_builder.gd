# grass_builder.gd
# Individual-blade grass system with dynamic chunk loading (hexaquo approach).
#
# Each blade is its own MultiMesh instance. Chunks are built only near
# the camera at 400 blades/m² (hexaquo density). Chunks load/unload
# as the player moves.
#
# 4 blade meshes: Lawn (4 tris), Wild (8), Shade (6), Sedge (6).
# Positions from ground_cover_instances.bin, expanded to individual blades.

var _loader
var _blade_meshes: Array = []
var _grass_shader: Shader
var _positions_by_chunk: Dictionary = {}  # "cx|cz" → Array of {wx, wz, wy, type, prox}
var _active_chunks: Dictionary = {}       # "blade_type|cx|cz" → MultiMeshInstance3D
var _last_camera_chunk := Vector2i(-9999, -9999)

const CHUNK := 10.0               # 10m chunks (smaller = finer loading)
const BLADES_PER_M2 := 400.0      # hexaquo density
const BLADE_SPREAD := 0.55        # random offset from grid position (m)
const VIS_RANGE := 18.0           # grass geometry visibility
const LOAD_RANGE := 25.0          # chunk load range (> vis for preloading)
const UNLOAD_RANGE := 35.0        # chunk unload range
...
func _build_grass() -> void:
	...
	_update_chunks_near(Vector3(-480, 0, 1020))  # Bethesda default spawn


func update_camera(camera_pos: Vector3) -> void:
	# Called each frame from main.gd — load/unload chunks near camera
...
func _update_chunks_near(pos: Vector3) -> void:
	...
	for key in _active_chunks:
		# key format: "blade_type|cx|cz", extract cx|cz
		var parts := key.split("|")
		var ck := "%s|%s" % [parts[1], parts[2]]
	...
	var built := 0
	for ck in needed:
		# Check if any blade type for this chunk is already loaded
...
func _build_chunk(ck: String) -> void:
	...
	# Calculate blade count: area of chunk's grass positions × density
	...
	# Blades per grid position to reach target density
	var blades_per_pos := clampi(target_blades, 10, 800)

	# Group blade instances by type
	...
	# Create MultiMesh per blade type in this chunk
