...
8. **Shadow cascade knobs — REVERSED & ADOPTED 2026-07-05** (woodland-perf-investigation Lever 3). The 2026-06-10 verdict (report 20260610_220421) that `--shadow-splits=2` "regresses (each split rasterizes more atlas area)" held in a RECEIVER/atlas-bound regime (lighter scene). At the repointed DENSE woodland poses on the real 6808-tree park (Ramble heading 328, North Woods heading 355), the frame is CASTER-bound — 6808 lod0 casters × cascade count — so halving cascades (4→2) HALVES shtri (Ramble 2.19→1.10 M, N.Woods 2.33→1.17 M): **North Woods −3.9 ms/+2fps, Ramble −1.9 ms/+1fps, the biggest single GPU win of that mission.** Walk-checked clean (no cascade swim/seam/acne; only a subtle mid-ground softening). **`SHADOW_PARALLEL_2_SPLITS` is now the shipped default** (main.gd; `--shadow-splits=4` restores 4). `--shadow-dist` (150→100) stayed dead at the dense poses too (casters near-field). `--shadow-blend=0` still within noise; PCF-q2/dapple policy unchanged.
