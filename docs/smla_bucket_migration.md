...
1. **`s/m/l` is a shared, species-wide structural schema, not a London-plane thing.**
   ...
   bucket names (s tier / m tier / l tier) are **London-plane
...
2. **The runtime recovers the species name by string-splitting the tier suffix.**
   ...
   name like `s tier` (space) or `upright_ovoid` (underscore) **breaks the
...
bucket identity as London-plane-only metadata.** Concretely: `s → s tier`,
`m → m tier`, `l → l tier`, recorded as a `crown_buckets` block on the
...
|---|---|---|---|
| A1 | `SPECIES["london_plane"]["tiers"]["s"]` | `target_h 9, height_range [7,13]`, skeleton_overrides (`sub_start_radius 0.7` weld, `sub_density 1.7`, `skeleton_max_depth 3`) | ✎ retarget to s tier: `target_h 10, height_range [7,12]`; params carry (see §3) |
| A2 | `…["tiers"]["m"]` | `target_h 22, height_range [15,25]`, `sub_start_radius 0.45`, `sub_density 1.14` | ✎ retarget to m tier: `target_h 15, height_range [12,18]` — **big drop, see §2** |
| A3 | `…["tiers"]["l"]` | `target_h 30, height_range [25,35]`, `card_rule_depth_keep {1:.04,2:.40,3:.62}` | ✎ retarget to l tier: `target_h 22, height_range [18,28]` — **big drop, see §2** |
| A4 | `…["tier_fraction"]` | `{"l":1.0, "m":0.40, "s":0.18}` | ✎ LFS (`l`) pull — **see §5 LOD budget** |
| A5 | `…["min_twig_diameter"]` | `{"s":0.04}` | ✓ keyed by slot; carries (s tier keeps the 4 cm sapling floor) |
...
|---|---|---|---|
| `s` = s tier | h ≤ 12 m | 9 | **10** |
| `m` = m tier | 12 < h ≤ 18 m | 22 | **15** |
| `l` = l tier | h > 18 m | 30 | **22** |

**How the existing authored 22 m and 30 m content is reassigned — stated explicitly:**

- **Old `_m` (target 22 m) does NOT survive as m tier content.** 22 m now falls in
  the **l tier** band (>18 m), not m tier (12–18). So slot `m` is
  **retargeted down to ~15 m** (the m tier centre, which is exactly what the
  ...
  **l tier**, not the new m tier.
- **Old `_l` (target 30 m) is reassigned DOWN to l tier (~22 m).** The real
...
|---|---|---|---|---|---|
| `s` | s tier | **0.35** | **0.80** | 10 | [7, 12] |
| `m` | m tier | **0.30** | **1.00** | 15 | [12, 18] |
| `l` | l tier | **0.20** | **1.20** | 22 | [18, 28] |
...
**s tier depth note (required scope §3):** the mould validation showed the young
bucket's hop **median is 5 — same as m tier — with a *tighter range* (3–5 vs 2–6)**,
...
## 4. s tier — no other extreme-scale surprises
...
## 5. LOD budget check — l tier (required scope)
...
At the inherited `tier_fraction l:1.0`, l tier is **≈ parity with the old `_l`
...
- **Honest caveat — do not apply blind:** the mould is **not yet wired into
  ...
  l tier `.glb` before it ships. That measurement is part of the
...
tiers["s"]: target_h 10, height_range [7,12]     # s tier  (was 9 / [7,13])
tiers["m"]: target_h 15, height_range [12,18]     # m tier     (was 22 / [15,25])
tiers["l"]: target_h 22, height_range [18,28]     # l tier (was 30 / [25,35])
tier_fraction: {"l": 0.60, "m": 0.40, "s": 0.18} # LFS pull 1.0→0.60 (§5, confirm by card count)
crown_buckets: {                                  # NEW form fields (§3)
  "s": {name:"s tier",     clear_bole_frac:0.35, aspect_wh:0.80},
  "m": {name:"m tier",        clear_bole_frac:0.30, aspect_wh:1.00},
  "l": {name:"l tier", clear_bole_frac:0.20, aspect_wh:1.20},
}
...
**Measured `_l` (l tier, target 22 m): 774 leaf clusters** (`h=26.3 m,
w=3.2 m`, seed 300). `_m` (m tier, target 15 m): **190 clusters** (`h=16.2 m,
...
if l tier's card budget ever needs pulling, that is the knob, and it should
...
- **Measured skeleton width is clamped ~3.0–3.2 m** for both tiers (`_m` w=3.0, `_l`
  ...
  (cards over-hang), but **nowhere near the l tier's aspect 1.2 wide spread.**
...
**Consequence:** step 6 ("verify m tier and l tier read as intended
