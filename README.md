...
### Vegetation
4-tier grass LOD system using three rendering methods: GPU particle per-blade instancing (0-22m), static MultiMesh crossed-card tuft chunks (15-55m), terrain shader impostor (40m+). Zone-aware color palettes across 10 landuse zones with shared patch noise for seamless transitions. 4 biome layers (lawn, shade, wild, sedge) with conservancy botanical data. Undergrowth and seasonal wildflowers via custom Blender scripts (MIT/CC0).
