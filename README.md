...
### Vegetation
4-tier grass LOD across three rendering methods: Tier 0 near-field GPU particles (0-6m, 12 blade variants + 5 botanical accents at 0.015m spacing), Tier 1 mid-field GPU particles (0-22m, 4 biome blades), Tier 2 static MultiMesh tuft chunks (15-55m), Tier 3 terrain shader impostor (40m+). Colors derived from published spectral reflectance curves via CIE 1931 + D65 colorimetry (ECOSTRESS/USGS data). Shared zone palette include (grass_zone_colors.gdshaderinc) drives all 3 tiers with identical multi-scale macro variation: 22m+9m field-scale splotchiness, 70m landscape gradient, 8m warm/cool hue drift, 3m micro patchiness. 4 biome layers (lawn/KBG, shade/fescue, wild/switchgrass, sedge/carex) with conservancy botanical data.
