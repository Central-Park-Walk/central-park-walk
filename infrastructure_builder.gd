## Infrastructure builder — rocks, labels, barriers, staircases, statues,
...
func _build_rocks(trees: Array, water: Array) -> void:
	...
	var rock_alb: ImageTexture = _loader._load_tex("res://textures/rock_wall_diff.jpg")
	var rock_nrm: ImageTexture = _loader._load_tex("res://textures/rock_wall_nrm.jpg")
	var rock_rgh: ImageTexture = _loader._load_tex("res://textures/rock_wall_rgh.jpg")
	...
	var rock_glb_meshes: Dictionary = _loader._load_glb_meshes(rock_glb_path)
	...
	var rock_mat: ShaderMaterial = _make_rock_material(rock_alb, rock_nrm, rock_rgh)
	...
	for i in range(0, trees.size(), 5):
		...
		var pt: Array = _loader._tree_pos(trees[i])
	...
	for vi in rock_variants.size():
		if not xforms_by_variant[vi].is_empty():
			_loader._spawn_multimesh(rock_variants[vi], rock_mat, xforms_by_variant[vi], "Rocks_%d" % vi)
...
func _make_rock_material(alb: ImageTexture, nrm: ImageTexture, rgh: ImageTexture) -> ShaderMaterial:
	...
	mat.shader = _loader._get_shader("rock", _rock_shader_code())
	if alb:
