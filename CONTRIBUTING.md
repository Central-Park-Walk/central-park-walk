...
1. **Additional tree species models**: We now have 17 custom Blender tree models (oak, elm, maple, birch, cherry, ginkgo, honeylocust, linden, london plane, callery pear, pine, willow, magnolia + generic deciduous + standing dead snag). More species detail welcome — subspecies variants, size classes, seasonal accuracy. Blender scripts follow the `scripts/make_*.py` pattern.
